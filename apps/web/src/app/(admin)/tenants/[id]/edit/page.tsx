...
export default function EditTenantPage() {
  ...
  const loadBedOptions = useCallback(async (roomId: string, keepBedId: string) => {
    if (!roomId) { setBedOptions(ALL_BEDS); return; }
    try {
      ...
      const beds = ALL_BEDS.slice(0, maxBeds).map((b) => {
        ...
        const occupiedByOther = !!bedMeta?.isOccupied && String(bedMeta.tenantId ?? '') !== String(id) && b.value !== keepBedId;
        return { value: b.value, label: occupiedByOther ? `${b.label} (Occupied)` : b.label, disabled: occupiedByOther };
      });
      setBedOptions(beds.filter((b) => !b.disabled || b.value === keepBedId).map(({ value, label }) => ({ value, label })));
    } catch { setBedOptions(ALL_BEDS.slice(0, 4)); }
  }, [id]);
  ...
  const onRoomChange = useCallback(async (roomId: string) => {
    ...
    const keepBed = roomId === ((tenantData?.room as { _id?: string } | undefined)?._id ?? tenantData?.roomId) ? currentBedId : '';
    if (!keepBed) setValue('bedId', '');
    try { const res = await api.get(`rooms/${roomId}`).json<{ success: boolean; data: RoomOption }>(); if (res.data.monthlyRent) setValue('monthlyRent', res.data.monthlyRent); } catch { /* ignore */ }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        monthlyRent: data.monthlyRent, depositPaid: data.depositPaid,
        bedId: data.bedId, roomId: data.roomId,
      ...
      router.push('/tenants');
    } catch { setSubmitError('Failed to update tenant. Check phone format (+91...) and try again.'); }
  };

  const err = errors as Record<string, { message?: string }>;

  return (
    <FormPage title="Edit Tenant" description="Update contact info, room assignment, rent, and documents" backHref="/tenants" error={submitError} isLoading={isLoading} maxWidth="4xl">
      <FormCard onSubmit={handleSubmit(onSubmit)} footer={<FormActions loading={isSubmitting} cancelHref="/tenants" submitLabel="Save Changes" divided={false} />}>
        ...
        <FormSection title="Room assignment" description="Only free beds (plus this tenant's current bed) appear for the selected room" divided>
          <FormGrid>
            <Controller name="roomId" control={control} render={({ field }) => (
              <ResourceSelect label="Room" endpoint="rooms?isActive=true" value={field.value} onChange={(val) => { field.onChange(val); void onRoomChange(val); }} placeholder="Select room..." error={err.roomId?.message} valueKey="_id"
            ...
            <Select label="Bed" options={bedOptions} error={err.bedId?.message} disabled={!roomIdWatch} {...register('bedId')} />
          </FormGrid>
        </FormSection>
