...
export default function EditMenuPage() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get(`menus/${id}`).json<{ success: boolean; data: FormData }>()
      .then((res) => { reset(res.data); setIsLoading(false); })
      .catch(() => { setSubmitError('Failed to load menu'); setIsLoading(false); });
  }, [id, reset]);
  ...
  return (
    <div className="animate-fade-in-up space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-surface-900 text-2xl font-extrabold">Edit Daily Menu</h2>
          <p className="text-surface-500 mt-0.5 text-sm">Update daily menu details</p>
        </div>
      </div>
