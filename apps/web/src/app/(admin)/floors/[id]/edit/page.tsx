...
const schema = z.object({
  ...
  floorNumber: z.coerce.number().int().positive('Floor number must be positive'),
...
export default function EditFloorPage() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get(`floors/${id}`).json<{ success: boolean; data: FormData }>()
      .then((res) => { reset(res.data); setIsLoading(false); })
      .catch(() => { setSubmitError('Failed to load floor'); setIsLoading(false); });
  }, [id, reset]);

  const onSubmit = async (data: FormData) => {
    setSubmitError('');
    try {
      await api.put(`floors/${id}`, { json: data }).json();
      router.push('/floors');
    } catch {
    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="rounded-lg border-[length:var(--bw-strong)] border-[color:var(--border-color)] bg-white p-6 shadow-[var(--shadow-card)]">
        <div className="space-y-5">
          ...
          <Input label="Floor Number" type="number" error={errors.floorNumber?.message} {...register('floorNumber')} />
          <Input label="Total Rooms" type="number" error={errors.totalRooms?.message} {...register('totalRooms')} />
        </div>
