...
export default function EditElectricityPage() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get(`electricity/${id}`).json<{ success: boolean; data: FormData }>()
      .then((res) => { reset(res.data); setIsLoading(false); })
      .catch(() => { setSubmitError('Failed to load electricity bill'); setIsLoading(false); });
  }, [id, reset]);

  const onSubmit = async (data: FormData) => {
    setSubmitError('');
    try {
      await api.put(`electricity/${id}`, { json: data }).json();
  ...
  return (
    <div className="animate-fade-in-up space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-surface-900 text-2xl font-extrabold">Edit Electricity Bill</h2>
          <p className="text-surface-500 mt-0.5 text-sm">Update electricity bill details</p>
        </div>
      </div>
